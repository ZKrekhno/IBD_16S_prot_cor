---
title: "Pathway-protein correlations"
...
pth_file_list <- str_subset(files,"clr(.)+pth.csv")
...
#pdf(here::here("spear_pathways/Scatter plots of top correlations for prevalent pathways.pdf"))
...
df_top_reduced <- keep(df_top_filt_list, ~nrow(.) > 0) %>% 
  map(~filter(.,abs(value.x)>0.52, value < 0.05))



#pdf(here::here("spear_pathways/Prevalent Pathway correlations.pdf"), height = 8, width = 10)
walk2(df_top_reduced, names(df_top_reduced),
      ~print(ggplot(.x, aes_(x=quote(Var1), y=quote(Var2))) +
              ...
              scale_fill_distiller(name = "Spearman rho",palette = "PuOr", breaks = c(-0.5,0,0.5)) +
              scale_color_distiller(name = "Spearman rho", palette = "PuOr",breaks = c(-0.5,0,0.5)) +
