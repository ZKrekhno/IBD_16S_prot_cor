...
#This allows to get the matrix with Pearson-like coefficients between bacteria and proteins
#Have to adjust the cut-off to detect anything for Entero
cutoff <- 0.3
a <- vector(mode = "list", length = 4)
for (i in seq_along(res.dblo)) {
  a [[i]] <- network(res.dblo [[i]], cutoff = cutoff, 
        color.node = c("lightblue","indianred1"), shape.node = c('circle','circle'),
        alpha.node = 0.5, symkey = T)
}
mat <- a [[4]] [[2]]
```

```{r auroc}
auroc_list <- list(rep(res.dblo, each = 2),
                   rep(names(res.dblo), each = 2),
                   rep(c("protein","microbiome"), times = length(res.dblo)))
#pdf(here::here(str_c("mixomics_res/AUC_ROC.pdf")))
pwalk(auroc_list,
      ~auroc(..1, roc.block = ..3, roc.comp = 2, title = str_c("ROC AUC for ",..2, " on ", ..3)))
#dev.off()
```
