...
names(meta_list_ibd_ti) <- c("Colon_Control","Colon_IBD", "Colon_IBD_with_TI","Colon_IBD_no_TI")
...
mq_bile_list <- meta_list_ibd_ti %>% 
  map(~ mq_bile_prot [pull(.x,var=sample),])
#Prepare otu counts matrices
entero_bile_list <- meta_list_ibd_ti %>% 
  map(~otu_mat_list [[1]] [pull(.x,var=sample), 
                     colnames(otu_mat_list [[1]]) %in% pull(filter(tax, Family == "f__Enterobacteriaceae"), var = "...1")])
#Confirm that things are in proper order
all(map2_lgl(entero_bile_list, mq_bile_list, ~all(rownames(.x) == rownames(.y))))

tidy_bile_list <- map2(mq_bile_list,entero_bile_list,
                       ~cbind(.x,.y)) %>%
  map(as.data.frame)
...
walk2(tidy_bile_graphing, names(tidy_bile_graphing),
      ~print(ggplot(.x, aes(x=.y, y = protein_abundance)) +
               ...
               labs(caption = .y, x="familyEntero Abundance")))
#dev.off()
