...
Create a matrix of just DA families

```{r clr-16s-DA-family-print, eval=FALSE}
otu_da <- read.csv(here::here("16S_res/Top hits triple DA - groups and tissue.csv")) %>% 
  filter(total_sig == 3)
otu_family <- otu_filt_clr [[3]] 
family_tax <- read.csv(here::here("corr_mats/Family_tax.csv"))
family_tax_da <- family_tax %>% 
  filter(Family %in% otu_da$Family) %>% 
  select(Family, X) %>% 
  deframe()
otu_da_family <- otu_family [family_tax_da,]
rownames(otu_da_family) <- names(family_tax_da)
write.csv(t(otu_da_family), here::here("corr_mats/DA_fam.csv"))
```
...
#Read in ALDEX2 results of the PICRUST2 analysis, and constrain to just significant hits
pth_da <- read_csv(here::here("picrust_res/Aldex2_functional_by_Group.csv"))
pth_da_signi <- pth_da %>% 
  filter(`q value t test` < 0.05 | `q value Wilcxon` < 0.05) %>% 
  pull(var = "rownames(x.tt)")

#Constrain matrix to just significant hits
pth_clr_da <- pth_clr [rownames(pth_clr) %in% pth_da_signi, ]
pth_log_da <- pth_prev [rownames(pth_clr) %in% pth_da_signi, ] %>% log1p
...
pth_clr_da %>% t %>% rowMeans() %>% hist(main = "Prevalent clr DA pathways")
pth_log_da %>% t %>% rowMeans() %>% hist(main = "Prevalent log DA pathways")
#write.csv(t(pth_clr), here::here("corr_mats/clr_pth.csv"))
#write.csv(t(pth_clr_da), here::here("corr_mats/clr_pth_da.csv"))
#write.csv(t(pth_log_da), here::here("corr_mats/log_pth_da.csv"))
```
