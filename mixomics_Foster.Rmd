...
walk2(res.dblo, names(res.dblo),~plotIndiv(.x, ind.names = F, 
          ...
          ellipse=F))
#dev.off()
walk2(res.dblo, names(res.dblo), 
      ~plotVar(.x, var.names = c(T, T),
        legend=TRUE, cutoff = 0.65, cex = c(2.5,2.5), col = c("cornflowerblue","firebrick3"),
        title = str_c(.y," Correlation Circle Plot")))
