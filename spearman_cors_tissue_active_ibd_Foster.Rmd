...
```{r select_top_prots}
#Import the results
da_res <- map(list.files(here::here("spear_subset_on_Foster"), pattern = "full"),
              ~read_csv( file = here::here(str_c("spear_subset_on_Foster/",.)))) 
da_res <- set_names(da_res, nm = list.files(here::here("spear_subset_on_Foster"), pattern = "full"))
...
#dev.off()
```
Plot specifically APOL2 and Enterobacteriace correlation in IBD Active IBD Ileum

```{r plot-APOL2-ENTERO}
#Select just protein and otu as selected top correlation
mq_select_list_apo <- set_names(mq_list,nm=dfs_names) %>% 
  keep(names(.) %in% names(df_top_cors))  %>% 
  map( ~ .x [,str_which(colnames(.x),"APOL2")]) %>%  
  map(as.data.frame, stringsAsFactors = F) %>% 
  keep(names(.) == "Family_Ileum_Yes")
#Get ID for Enterobacteriaceae
entero_id <- df_tax_final %>% 
  keep(names(.) == "Family_Ileum_Yes") %>% 
  map(filter, str_detect(Family,"Enterobacteriaceae")) %>% 
  map(pull,var = "Var2")
otu_select_list_entero <- set_names(otu_filt_list,nm=dfs_names) %>% 
  keep(names(.) %in% names(df_top_cors)) %>% 
  keep(names(.) == "Family_Ileum_Yes") %>% 
  map2(entero_id, ~ .x [,.y]) %>% 
  map(as.data.frame, stringsAsFactors = F) 
#prepare list for scatter plots
scatter_list_indiv <- map2(mq_select_list_apo, otu_select_list_entero, merge, by = 0)

#svg(here::here("spear_subset_on_Foster/APOL2-Entero filteredFamily_Ileum_Yes Scatter.svg"), width = 6, height = 4)
pwalk(list(scatter_list_indiv, names(scatter_list_indiv)), 
      ~print(ggplot(..1, aes(x = `.x[[i]].x`, y = `.x[[i]].y`)) +
               geom_point() +
               geom_smooth(method = "lm") + 
               labs(x = "APOL2", y = "Enterobacteriaceae")))
